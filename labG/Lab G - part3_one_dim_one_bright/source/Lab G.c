...
#define LED_PERIOD_TICKS (150000)	// total ticks in counter in which two IRQ made
// 30,000,000 is really slow. Visible to human eye.
#define INIT_COUNT_IRQ_CH0 (LED_PERIOD_TICKS/2)
#define INIT_PWM_DUTY_FACTOR 0.05
...
void MRT0_IRQHandler(void){
	...
	if(chan == 0){
		...
		if (pwm_state_ch1) // Tun on the LED if it's the ON time for PWM
		{
			// Turn off PIO0_8 ... which turns on LED
			...
			// Bit 31: 0 for loading during next cycle
			// 1 for loading immediately.
			MRT0->CHANNEL[MRT_CHAN1].INTVAL = (uint32_t)(LED_PERIOD_TICKS * (duty_factor))
											& ~(MRT_CHANNEL_INTVAL_LOAD_MASK);
			// Now, change the state for the *next* time the IRQ fires.
			pwm_state_ch1 = 0; // Next time, it's "off time"
		}else{ // Turn off the LED if it's the OFF time for PWM
			GPIO->SET[0] = (1UL<<LED_USER2); // Turn on PIO0_8 .. which turns off LED
			...
			// Bits 30:0 => a non-zero value in bits 30:0 will start the MRt
			// Bits 31 => 0 for loading during next cycle; 1 for immediate loading.
			MRT0->CHANNEL[MRT_CHAN1].INTVAL = (uint32_t)(LED_PERIOD_TICKS * (1-duty_factor))
											  & ~(MRT_CHANNEL_INTVAL_LOAD_MASK);

			// Now, change the state for the *next* time the IRQ fires.
			pwm_state_ch1 = 1; // next time, it's "on time"
		}
	}
...
int main(void) {
	...
	MRT0->CHANNEL[MRT_CHAN1].CTRL = (MRT_REPEAT << MRT_CHANNEL_CTRL_MODE_SHIFT	// Bit 1 is 0
									 | MRT_CHANNEL_CTRL_INTEN_MASK);			// Bit 0 is 1
	...
	MRT0->CHANNEL[MRT_CHAN1].INTVAL = INIT_COUNT_IRQ_CH0				// count value
									& ~(MRT_CHANNEL_INTVAL_LOAD_MASK);	// clear bit 31
